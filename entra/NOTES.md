...
## 1. Placeholder checklist — every value you must supply

| Placeholder | What it is | Where you get it | Used in |
| --- | --- | --- | --- |
| `<APP_A_CLIENT_ID>` | Teams bot app's Application (client) ID | Created in 01 step 1; read off the Overview blade | 01, 02, 03, 04, manifest |
| `<APP_A_CLIENT_SECRET>` | App A client secret **Value** | Created in 01 step 3, shown once | 02, 04 — **Google Secret Manager only** |
| `<BOT_DOMAIN>` | Public HTTPS hostname of the middle tier, bare (no scheme) | Your hosting choice | 01 step 7, manifest `validDomains` and `developer.*Url` |
| `<TEAMS_APP_GUID>` | Identifier for the Teams app itself | `uuidgen`; **not** App A's client ID | `manifest.json` `id` |
| `<TEAMS_SSO_TOKEN>` | The user assertion fed into OBO | 04 step 1a (device-code stand-in) or 1b (real, from the invoke activity) | 02, 04 |
| `<REGION>` | Agent Runtime region | Your deployment | 04 step 6 |
| `<REASONING_ENGINE_ID>` | Reasoning Engine numeric ID | Your deployment | 04 step 6 |
...
### Values that are known and must NOT be changed

| Value | What |
| --- | --- |
| `<ENTRA_TENANT_ID>` | Entra tenant ID |
| `<FEDERATION_APP_CLIENT_ID>` | **App B**, the federation app. Exists. Do not recreate. |
| `<ANALYST_OBJECT_ID>` | `analyst@<TENANT_DOMAIN>` object ID |
| `teams-bot-demo` / `entra` | Google workforce pool / provider |
| `<GCP_PROJECT_ID>` / `organizations/<GCP_ORG_ID>` | GCP project / org |
| `https://login.microsoftonline.com/<ENTRA_TENANT_ID>/v2.0` | Provider issuer |
...
2. **The audience risk is narrower and more actionable than the brief framed it.**
   ...
   reframed throughout as one specific, checkable, fixable condition —
   `requestedAccessTokenVersion` — rather than as an open uncertainty.
